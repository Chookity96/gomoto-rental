...
function Fleet({}) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const { data } = await axios.get(
        `https://62aa09d13b3143855440da67.mockapi.io/api/fleetdata/fleetdata`,
      )
      setBikeData(data)
    }
    fetchData()
  }, [])
  const filteredCategoryData = bikeData.find(
    (bikes) => bikes.category === category,
  )
  const allbikes = []
  ...
  useEffect(() => {
    ...
    filter.forEach((category) => {
      bikeData
        .find((bikes) => bikes.category === category)
        ?.bikes.forEach((bikes) =>
          setFilteredData((filteredData) => [...filteredData, bikes]),
        )
    })
  }, [filter])
  ...
  useEffect(() => {
    ...
    window
      ...
      .addEventListener('change', function (mm) {
        if (mm.matches) {
    ...
    <div className="bg-[#001951]">
      ...
      <div className="w-full max-w-6xl mx-auto p-5 space-y-4 lg:flex lg:space-y-0 lg:pt-16">
        <div className="flex items-center justify-between">
          <h1 className="text-[#19f9b9] text-4xl font-bold lg:hidden">
            Filter
          </h1>
          {showFilter ? (
            <XCircleIcon
              className="text-[#19f9b9] h-10 cursor-pointer lg:hidden"
            ...
            <ul className="list-none text-white">
              <li className="flex m-4 space-x-2 items-center">
                <input
                  ...
                  type="checkbox"
                  id="check1"
                  defaultChecked={filter.includes('street') && true}
                  onClick={(e) => {
                    e.target.checked
                      ? setFilter((filter) => [...filter, 'street'])
                      : setFilter((filter) =>
                          filter.filter((cat) => cat !== 'street'),
                        )
                  }}
                />
                <label className="tracking-wider select-none">
                  <span></span>Street
                </label>
              </li>
              <li className="flex m-4 space-x-2 items-center">
                <input
                  ...
                  type="checkbox"
                  id="check2"
                  defaultChecked={filter.includes('sports') && true}
                  onClick={(e) => {
                    e.target.checked
                      ? setFilter((filter) => [...filter, 'sports'])
                      : setFilter((filter) =>
                          filter.filter((cat) => cat !== 'sports'),
                        )
                  }}
                />
              ...
              <li className="flex m-4 space-x-2 items-center">
                <input
                  ...
                  type="checkbox"
                  id="check3"
                  defaultChecked={filter.includes('touring') && true}
                  onClick={(e) => {
                    e.target.checked
                      ? setFilter((filter) => [...filter, 'touring'])
                      : setFilter((filter) =>
                          filter.filter((cat) => cat !== 'touring'),
                        )
                  }}
                />
                <label className="tracking-wider">
              ...
              <li className="flex m-4 space-x-2 items-center">
                <input
                  ...
                  type="checkbox"
                  id="check4"
                  defaultChecked={filter.includes('motocross') && true}
                  onClick={(e) => {
                    e.target.checked
                      ? setFilter((filter) => [...filter, 'motocross'])
                      : setFilter((filter) =>
                          filter.filter((cat) => cat !== 'motocross'),
                        )
                  }}
                />
                <label className="tracking-wider">
        ...
        <div className="flex flex-wrap w-full justify-between md:justify-start">
          {(!(filter.length > 0) ? allbikes : filteredData).map(
            (bike, index) => (
              <div
                key={index}
                id={bike.id}
                className="bg-[#2730ed] w-[100%] max-w-[280px] mx-auto xs:max-w-none xs:w-[45%] rounded-xl z-10 cursor-pointer xs:mx-2 my-3 md:w-[30%] shadow-lg"
                onClick={(e) => {
                  navigate.push(`/fleet/bikes/${bike.id}`)
                }}
              >
